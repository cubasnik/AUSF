...
def main() -> int:
    ...
    try:
        ...
        third_refreshed_context = client.get_authentication_context(challenge["authCtxId"])
        assert third_refreshed_context["status"] == "CHALLENGE_SENT"
        assert third_refreshed_context["5gAuthData"]["rand"] == third_resync["5gAuthData"]["rand"]
        print(f"third-refreshed-context: {third_refreshed_context}")
