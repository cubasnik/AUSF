...
def main() -> int:
    ...
    try:
        ...
        auts = control_plane_context["auts"]

        try:
            client._call(
                "POST",
                f"/nausf-auth/v1/ue-authentications/{challenge['authCtxId']}/5g-aka-confirmation",
                {"resStar": "deadbeef", "auts": auts},
            )
        except AUSFError as error:
            assert error.status_code == 400
            assert error.payload["cause"] == "INVALID_CONFIRMATION_PAYLOAD"
            assert error.payload["detail"] == "exactly one of resStar or auts is required for 5g-aka-confirmation and eapPayload must be omitted"
            print(f"ambiguous-confirmation-rejected-error: {error.payload}")
        else:
            raise AssertionError("expected 400 INVALID_CONFIRMATION_PAYLOAD for ambiguous 5G_AKA confirmation payload")
        ...
        final_confirmed = client.confirm_authentication(
            challenge["authCtxId"],
            control_plane_context["xresStar"],
        )
        ...
        assert matching_notification["authType"] == "5G_AKA"
