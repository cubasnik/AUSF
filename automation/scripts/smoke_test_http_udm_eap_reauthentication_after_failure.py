...
def main() -> int:
    ...
    try:
        ...
        stale_eap_ctx = get_eap_context(supi, challenge["eapSession"]["payload"])
        stale_payload = build_eap_aka_prime_response_payload(initial_control_plane_context["xresStar"], stale_eap_ctx)
        try:
            ...
            raise AssertionError(
                "expected 401 AUTHENTICATION_REJECTED for stale eapPayload after re-auth refresh"
            )
        ...
        try:
            ...
            raise AssertionError(
                "expected 401 AUTHENTICATION_REJECTED for re-authentication after failure"
            )
