...
def main() -> int:
    ...
    try:
        ...
        try:
            client.confirm_authentication_with_auts(challenge["authCtxId"], initial_control_plane_context["auts"])
        ...
        try:
            client.confirm_authentication(challenge["authCtxId"], refreshed_control_plane_context["xresStar"])
        except AUSFError as error:
            assert error.status_code == 401
            assert error.payload["cause"] == "AUTHENTICATION_REJECTED"
            ...
            raise AssertionError(
                "expected 401 AUTHENTICATION_REJECTED for confirmation after sync-failure failure"
            )
