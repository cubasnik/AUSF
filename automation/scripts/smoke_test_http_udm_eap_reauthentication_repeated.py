...
def main() -> int:
    ...
    try:
        ...
        print(f"first-reauth-confirmed: {first_reauth}")
        ...
        second_reauth = client.confirm_eap_authentication(
            challenge["authCtxId"],
            build_eap_aka_prime_reauthentication_payload(get_eap_context(supi, first_reauth["eapSession"]["payload"])),
        )
        print(f"second-reauth-confirmed: {second_reauth}")
