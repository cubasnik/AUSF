...
def main() -> int:
    ...
    missing_auth_ctx_id = f"{challenge['authCtxId']}-missing"
    try:
        client.confirm_authentication(missing_auth_ctx_id, "deadbeef")
        ...
        print(f"missing-context-error: {error.payload}")
    else:
        raise AssertionError("expected 404 CONTEXT_NOT_FOUND for missing authentication context")
    finally:
