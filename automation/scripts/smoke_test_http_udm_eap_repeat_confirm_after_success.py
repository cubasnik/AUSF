...
from smoke_runtime import AUSF_BASE_URL, MOCK_AMF_BASE_URL, MOCK_NRF_BASE_URL, MOCK_UDM_BASE_URL, build_eap_aka_prime_response_payload, get_eap_context, load_amf_notifications, load_control_plane_authentication_context, wait_for_health
...
def main() -> int:
    ...
    try:
        ...
        try:
            client.confirm_eap_authentication(challenge["authCtxId"], eap_payload)
            ...
            print(f"repeat-confirm-rejected-error: {error.payload}")
        else:
            raise AssertionError("expected 401 AUTHENTICATION_REJECTED for repeated EAP confirmation after success")

        stored_context = client.get_authentication_context(challenge["authCtxId"])
        assert stored_context["status"] == "AUTHENTICATED"
        notifications = load_amf_notifications()[baseline_notification_count:]
        assert len([notification for notification in notifications if notification["authCtxId"] == challenge["authCtxId"]]) == 1
