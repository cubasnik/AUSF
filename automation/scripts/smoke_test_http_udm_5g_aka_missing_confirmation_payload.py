...
def main() -> int:
    ...
    try:
        ...
        try:
            ...
            assert error.payload["cause"] == "MANDATORY_IE_MISSING"
            assert error.payload["detail"] == "resStar or auts is required"
            print(f"missing-confirmation-payload-error: {error.payload}")
        else:
            raise AssertionError("expected 400 MANDATORY_IE_MISSING for empty 5G_AKA confirmation payload")
