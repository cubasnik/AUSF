...
def main() -> int:
    ...
    try:
        ...
        first_resync = client.confirm_authentication_with_auts(challenge["authCtxId"], first_control_plane_context["auts"])
        ...
        second_res_star = second_control_plane_context["xresStar"]

        second_resync = client.confirm_authentication_with_auts(challenge["authCtxId"], second_control_plane_context["auts"])
        ...
        third_resync = client.confirm_authentication_with_auts(challenge["authCtxId"], third_control_plane_context["auts"])
        print(f"third-sync-failure-confirmed: {third_resync}")
        assert third_resync["authResult"] == "SYNC_FAILURE"

        try:
            client.confirm_authentication(challenge["authCtxId"], second_res_star)
            ...
            print(f"stale-third-refresh-authentication-rejected-error: {error.payload}")
        else:
            raise AssertionError(
                "expected 401 AUTHENTICATION_REJECTED for stale resStar after third 5G_AKA re-sync refresh"
            )

        stored_context = client.get_authentication_context(challenge["authCtxId"])
        assert stored_context["status"] == "FAILED"
        assert stored_context["5gAuthData"]["rand"] == third_resync["5gAuthData"]["rand"]
