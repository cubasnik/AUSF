...
def main() -> int:
    ...
    try:
        ...
        refreshed = client.confirm_authentication_with_auts(challenge["authCtxId"], initial_control_plane_context["auts"])
        print(f"sync-failure-confirmed: {refreshed}")
        assert refreshed["authResult"] == "SYNC_FAILURE"

        refreshed_control_plane_context = load_control_plane_authentication_context(supi)

        stale_res_star = initial_control_plane_context["xresStar"]
        try:
            client.confirm_authentication(challenge["authCtxId"], stale_res_star)
            ...
            print(f"stale-authentication-rejected-error: {error.payload}")
        else:
            raise AssertionError("expected 401 AUTHENTICATION_REJECTED for stale resStar after 5G_AKA re-sync refresh")

        try:
            client.confirm_authentication_with_auts(challenge["authCtxId"], refreshed_control_plane_context["auts"])
        except AUSFError as error:
            assert error.status_code == 401
            assert error.payload["cause"] == "AUTHENTICATION_REJECTED"
            assert error.payload["detail"] == "authentication context is no longer pending"
            print(f"repeat-sync-failure-rejected-error: {error.payload}")
        else:
            raise AssertionError(
                "expected 401 AUTHENTICATION_REJECTED for synchronization-failure after failure"
            )
