...
def main() -> int:
    ...
    try:
        ...
        stale_payload = build_eap_aka_prime_response_payload(first_refreshed_control_plane_context["xresStar"])

        second_refresh = client.confirm_eap_authentication(
            challenge["authCtxId"],
            build_eap_aka_prime_synchronization_failure_payload(),
        )
        print(f"second-sync-failure-confirmed: {second_refresh}")
        ...
        try:
            ...
            raise AssertionError(
                "expected 401 AUTHENTICATION_REJECTED for stale eapPayload after second sync-failure refresh"
            )
