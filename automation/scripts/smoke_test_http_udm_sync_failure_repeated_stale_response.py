...
def main() -> int:
    ...
    try:
        ...
        first_res_star = first_control_plane_context["xresStar"]
        ...
        try:
            client.confirm_authentication(challenge["authCtxId"], first_res_star)
            ...
            raise AssertionError(
                "expected 401 AUTHENTICATION_REJECTED for stale resStar after second 5G_AKA re-sync refresh"
            )

        stored_context = client.get_authentication_context(challenge["authCtxId"])
        assert stored_context["status"] == "FAILED"
        assert stored_context["5gAuthData"]["rand"] == second_resync["5gAuthData"]["rand"]
