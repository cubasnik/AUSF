...
def main() -> int:
    ...
    confirmed = client.confirm_authentication_with_auts(challenge["authCtxId"], auts)
    print(f"sync-failure-confirmed: {confirmed}")

    assert confirmed["authCtxId"] == challenge["authCtxId"]
    assert confirmed["authResult"] == "SYNC_FAILURE"
    assert "5gAuthData" in confirmed
    assert confirmed["5gAuthData"]["rand"] != challenge["5gAuthData"]["rand"]
    assert confirmed["5gAuthData"]["autn"] != challenge["5gAuthData"]["autn"]

    refreshed_context = client.get_authentication_context(challenge["authCtxId"])
    assert refreshed_context["status"] == "CHALLENGE_SENT"
    assert refreshed_context["5gAuthData"]["rand"] == confirmed["5gAuthData"]["rand"]
    ...
    final_confirmed = client.confirm_authentication(
        challenge["authCtxId"],
        refreshed_control_plane_context["xresStar"],
    )
