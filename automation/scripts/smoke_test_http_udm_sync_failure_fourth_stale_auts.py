...
def main() -> int:
    ...
    try:
        ...
        try:
            client.confirm_authentication_with_auts(challenge["authCtxId"], third_auts)
            ...
            print(f"stale-fourth-refresh-auts-authentication-rejected-error: {error.payload}")
        else:
            raise AssertionError(
                "expected 401 AUTHENTICATION_REJECTED for stale AUTS after fourth 5G_AKA re-sync refresh"
            )

        stored_context = client.get_authentication_context(challenge["authCtxId"])
        assert stored_context["status"] == "FAILED"
        assert stored_context["5gAuthData"]["rand"] == fourth_resync["5gAuthData"]["rand"]
