...
def main() -> int:
    ...
    try:
        ...
        third_control_plane_context = load_control_plane_authentication_context(supi)
        third_auts = third_control_plane_context["auts"]

        third_resync = client.confirm_authentication_with_auts(challenge["authCtxId"], third_auts)
        print(f"third-sync-failure-confirmed: {third_resync}")
        assert third_resync["authCtxId"] == challenge["authCtxId"]
        assert third_resync["authResult"] == "SYNC_FAILURE"
        assert third_resync["5gAuthData"]["rand"] != second_resync["5gAuthData"]["rand"]
        assert third_resync["5gAuthData"]["autn"] != second_resync["5gAuthData"]["autn"]

        notifications = load_amf_notifications()[baseline_notification_count:]
        assert not notifications
        print("amf-notifications-after-third-refresh: []")

        fourth_control_plane_context = load_control_plane_authentication_context(supi)
        fourth_auts = fourth_control_plane_context["auts"]

        fourth_resync = client.confirm_authentication_with_auts(challenge["authCtxId"], fourth_auts)
        print(f"fourth-sync-failure-confirmed: {fourth_resync}")
        assert fourth_resync["authCtxId"] == challenge["authCtxId"]
        assert fourth_resync["authResult"] == "SYNC_FAILURE"
        assert fourth_resync["5gAuthData"]["rand"] != third_resync["5gAuthData"]["rand"]
        assert fourth_resync["5gAuthData"]["autn"] != third_resync["5gAuthData"]["autn"]

        fourth_refreshed_context = client.get_authentication_context(challenge["authCtxId"])
        assert fourth_refreshed_context["status"] == "CHALLENGE_SENT"
        assert fourth_refreshed_context["5gAuthData"]["rand"] == fourth_resync["5gAuthData"]["rand"]
        print(f"fourth-refreshed-context: {fourth_refreshed_context}")

        notifications = load_amf_notifications()[baseline_notification_count:]
        assert not notifications
        print("amf-notifications-after-fourth-refresh: []")
