...
| # | Задача | Уровень | Описание пробела |
|---|--------|---------|------------------|
| Г | **PostgreSQL production hardening** | Java / Инфраструктура | БД работает, но нет: schema migrations (Flyway/Liquibase), read-replica / HA, backup/restore, управления credentials через Vault или Kubernetes Secrets |
| Д | **Production Namf_Communication** | Go | NRF heartbeat реализован; production Namf подписка на события аутентификации, retry-логика уведомлений и очередь доставки — отсутствуют |
| Е | **Mutual TLS, ротация сертификатов (автоматическая)** | Go + Java | mTLS реализован; автоматическая ротация и SPIFFE/SVID — нет |
