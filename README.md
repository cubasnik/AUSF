...
| Слой | Язык | Технологии | Назначение |
|------|------|-----------|------------|
| **Go-микросервис** (`microservices/`) | Go 1.25 | net/http, Redis, Prometheus | SBI-фасад AUSF: принимает запросы от AMF, проксирует в control-plane, нотификации Namf |
| **Control-plane** (`control-plane/`) | Java 25 / Spring Boot 4.0.6 | JPA/Hibernate 7, Flyway 11, Micrometer OTel | Бизнес-логика: 5G-AKA, EAP-AKA', SIDF, SoR/UPU, UDM-клиент, NRF lifecycle |
| **Networking** (`networking/`) | C++17 | CMake, Winsock2/POSIX UDP | PFCP-сессии (TS 29.244), GTP-U кодек, DataPlane (PDR/FAR), usage reporting |
| **Автоматизация** (`automation/`) | Python 3.10+ | requests, Docker Compose | ~80 smoke-тестов, load-тест, TLS/Redis-failover сценарии |

### Реализованные интерфейсы

| Интерфейс | Спецификация | Статус |
|-----------|-------------|--------|
| Nausf_UEAuthentication — create/confirm/delete, 5G-AKA + EAP-AKA', SYNC_FAILURE re-sync, SUCI де-конкрементация (X25519/P-256/null-scheme), SoR/UPU-protection | TS 29.509 Clause 6.1–6.3 | ✅ 100% |
| Nnrf_NFManagement — регистрация/heartbeat/дерегистрация, подписки на статус NF | TS 29.510 | ✅ 100% |
| Nnrf_NFDiscovery — discovery UDM и других NF, subscription-based callbacks | TS 29.510 | ✅ 100% |
| Nudm_UEAuthentication — получение auth-векторов, PUT/DELETE auth-events | TS 29.503 | ✅ 100% |
| Namf_Communication — callback уведомления о результате аутентификации, retry ×3 | TS 29.518 | ✅ 100% |

### Безопасность

| Возможность | Статус |
|-------------|--------|
| TLS 1.2+/mTLS (inbound + outbound), cert hot-reload (SIGHUP), OCSP best-effort | ✅ |
| OAuth2 JWT: проверка exp/aud/scope, JWKS RS256/PS256/ES256, RFC 7662 token introspection, outbound CCF (client_credentials) | ✅ |
| HTTP/2 h2c + h2 via ALPN, graceful shutdown (GOAWAY) | ✅ |
| Circuit breaker на UDM и control-plane (configurable threshold/timeout) | ✅ |
| Retry с exponential backoff на transient 5xx | ✅ |
| CVE-статус зависимостей (проверено 2026-05-13) | ✅ 0 уязвимостей |

### Хранение состояния

- **Go**: `InMemoryAuthContextStore` (по умолчанию) / `FileAuthContextStore` (persist при рестарте) / **Redis** (Namf retry queue, опционально через `docker-compose.redis.yml`)
- **Java**: PostgreSQL + Hibernate 7 + Flyway миграции, HikariCP connection pool

### Тестирование

| Уровень | Файлов | Покрытие |
|---------|--------|---------|
| Java unit-тесты | 11 | `AuthenticationManager`, `Milenage`, `SuciDeconcealer`, `Tuak`, `NnrfClient`, `UdmService`, JPA-репозиторий |
| Go unit-тесты | 16 | `AuthService`, JWT/JWKS/OCSP, authorization, HTTP handler, overload, tracing |
| Python smoke-тесты | ~80 | 5G-AKA happy path, EAP-AKA', все негативные сценарии, SYNC_FAILURE (unlimited), TLS, Redis failover, chaos |

### Технический стек

| Компонент | Версия |
|-----------|--------|
| Java / Spring Boot | 25 / 4.0.6 |
| Hibernate ORM / Flyway | 7.2.12 / 11.14.1 |
| Micrometer / OpenTelemetry SDK | 1.16.5 / 1.55.0 |
| PostgreSQL driver | 42.7.11 |
| Go | 1.25 |
| Redis client (go-redis) | v9.19.0 |
| Prometheus Go client | 1.23.2 |
| C++ standard | C++17 |
| Python | 3.10+ |
