...
Ниже перечислено всё, что **ещё не реализовано** на момент последнего релиза `vAUSFver.1.0`, сгруппировано по уровням.
...
| Б | **Полный конечный автомат EAP-AKA' по TS 33.501** | Go + Java | Минимальные потоки sync-failure, re-auth и fast re-auth работают. Не покрыты: корректное форматирование EAP-нагрузок (AT_MAC, AT_RAND, AT_AUTN и т.д.), полная семантика состояний TS 33.501 §6.1.3, интероперабельность с реальным UE |

### Горизонт 2 — Надёжность и наблюдаемость

| # | Задача | Уровень | Описание пробела |
|---|--------|---------|-----------------|
| В | **Распределённая трассировка OpenTelemetry (полная)** | Go + Java | `X-Trace-Id` и `traceparent` передаются в заголовках ответов Go, но полноценный OTel SDK (spans, exporters, OTLP/Jaeger) не подключён ни в Go, ни в Java |
| Г | **Нагрузочное и soak-тестирование** | Автоматизация | Нет ни одного нагрузочного теста; пропускная способность, поведение TTL под конкурентной нагрузкой и производительность записи в файловое хранилище контекстов не измерялись |

### Горизонт 3 — Интеграция и инфраструктура

| # | Задача | Уровень | Описание пробела |
|---|--------|---------|-----------------|
| Д | **Производственная совместимость с Nudm (TS 29.503 полная)** | Java | HTTP-клиент UDM упрощён под текущий mock-контракт; реальный `Nudm_UEAuthentication` возвращает вложенный `authVector`, разные форматы ответов и дополнительные поля согласно TS 29.503 |
| Е | **Реальная интеграция плоскости данных PFCP (networking/)** | C++ | Весь `networking/` — заглушка (~85% milestone: GTP-U codec, PFCP правила хранятся); нет подключения к реальному UDP-сокету и к pipeline результатов аутентификации Go/Java |
| Ж | **Mutual TLS, ротация сертификатов, OAuth2 client credentials** | Go + Java | Реализован односторонний TLS + опциональный Bearer-токен; mTLS (клиентские сертификаты для `nausf-auth`), автоматическая ротация сертификатов и OAuth2 client credentials flow — отсутствуют |
| З | **PostgreSQL production hardening** | Java / Инфраструктура | БД работает, но нет: schema migrations (Flyway/Liquibase), read-replica / HA, backup/restore, управления credentials через Vault или Kubernetes Secrets |
| И | **Production Namf_Communication** | Go | NRF heartbeat реализован; production Namf подписка на события аутентификации, retry-логика уведомлений и очередь доставки — отсутствуют |
